...
def op_checksig(stack, z):
    if len(stack) < 2:
        raise ValueError("Insufficient items on the stack for OP_CHECKSIG")

    sec_pubkey = stack.pop()
    der_signature = stack.pop()[:-1]  # Remove the last byte (hash_type)
    ...
    # Verify the signature using S256Point.verify()
    is_valid_signature = pubkey.verify(z, signature)

    # Push an encoded 1 or 0 depending on whether the signature verified
    stack.append(encode_num(int(is_valid_signature)))
